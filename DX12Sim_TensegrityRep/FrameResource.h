...
struct InstanceItemCounts {
	UINT instanceGroupCount = 0;
	UINT maxCounts[10] = { 0 };
	char name[10][64] = { "" };
};

const UINT instDataSize = sizeof(InstanceData);
...
public:
	...
	FrameResource(ID3D12Device* device, UINT passCount, InstanceItemCounts items, bool flexiGrid,
		UINT totalDyVertexCount, UINT materialCount);
   ...
   // std::unique_ptr<UploadBuffer<FrameConstants>> FrameCB = nullptr;
	std::unique_ptr<UploadBuffer<PassConstants>> PassCBL = nullptr;
	std::unique_ptr<UploadBuffer<PassConstants>> PassCBR = nullptr;
	...
	std::unique_ptr<UploadBuffer<InstanceData>> InstanceBuffer[10] = { nullptr }; // 10 was chosen at random relates to the InstanceItemCounts

	// We cannot update a dynamic vertex buffer until the GPU is done processing
	// the commands that reference it.  So each frame needs their own.
	std::unique_ptr<UploadBuffer<Vertex>> DynamicVB = nullptr;

	Microsoft::WRL::ComPtr<ID3D12QueryHeap> mTimestampQueryHeaps;
	Microsoft::WRL::ComPtr<ID3D12Resource> mTimestampResultBuffers;
