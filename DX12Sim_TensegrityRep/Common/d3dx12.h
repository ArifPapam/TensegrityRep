...
struct CD3DX12_RASTERIZER_DESC : public D3D12_RASTERIZER_DESC
{
    ...
    {}
    explicit CD3DX12_RASTERIZER_DESC( CD3DX12_DEFAULT ) //This is shown on https://msdn.microsoft.com/en-us/library/windows/desktop/dn770387(v=vs.85).aspx
    {
        FillMode = D3D12_FILL_MODE_SOLID;
        CullMode = D3D12_CULL_MODE_BACK;
        FrontCounterClockwise = FALSE;
        DepthBias = D3D12_DEFAULT_DEPTH_BIAS;// Used with shadows and other things
        DepthBiasClamp = D3D12_DEFAULT_DEPTH_BIAS_CLAMP; // Helps avoid depth bias artifacts 
        SlopeScaledDepthBias = D3D12_DEFAULT_SLOPE_SCALED_DEPTH_BIAS; // For the above
        DepthClipEnable = TRUE; // if FALSE:  you can avoid complex special-case handling for geometry that goes beyond the back clipping plane (at the risk of atrifacts)
        MultisampleEnable = FALSE; // quadrilateral or alpha line anti-aliasing algorithm on multisample antialiasing (MSAA) render targets
        AntialiasedLineEnable = FALSE; //Complex see table https://msdn.microsoft.com/en-us/library/windows/desktop/ff476198(v=vs.85).aspx
        ForcedSampleCount = 0; // Valid values are 0, 1, 2, 4, 8, and optionally 16
