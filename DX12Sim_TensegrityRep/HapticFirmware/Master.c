 ...
 ****************************************************************************/
/*
 * 東工大共同研究HapticsDeviceの制御用にモノワイヤレス株式会社によるApp_Tweliteをtsutohが改変。
 * 開発環境およびソースコードはモノワイヤレス株式会社提供のTWESDK_201408-31による。
 * 本TWE-LITE SDKの添付物（ソースコード、ライブラリ、資料）については、別途ライセンス記述があるものは
 * その記述に従った上で、モノワイヤレスソフトウェア使用許諾契約書(MW-SLA-1、
 * https://sdk.twelite.info/overview/mwsla)に基づき取り扱う。
 */
...
int16 qu[4];
bool DMPwork=FALSE;
volatile int16 Mtimer[6][3];
volatile uint8 Mpwm[16];
volatile uint8 Mpwmwait[16];
volatile uint8 Mdrv[4];
volatile bool MisLeft=FALSE;
volatile bool MisRight=FALSE;

const uint16 init_PWM_Hz[4]={100,128,128,50};	//Set for prescale
uint16 HARDTOUCHPOS,RELEASEPOS,CLIB,PUNCH,MPWM_WAIT;
...
void cbAppColdStart(bool_t bStart) {
	if (!bStart) {
		...
		if ( ((uint16)(sAppData.u8AppLogicalId>>4)&0x0f)==0x03){
			MisRight=TRUE;
			vfPrintf(&sSerStream,"ID=%d Righthand"LB,sAppData.u8AppLogicalId);
		}
		if ( ((uint16)(sAppData.u8AppLogicalId>>4)&0x0f)==0x04){
			MisLeft=TRUE;
			vfPrintf(&sSerStream,"ID=%d Lefthand"LB,sAppData.u8AppLogicalId);
		}
...
void cbToCoNet_vMain(void) {
  ...
  // Check for new data in the FIF
	//2017年版
	//右手	Cid=0x00+1~6 	親指、人差指、中指、薬指、子指、手首
	//左手	Cid=0x00+11~16	親指、人差指、中指、薬指、子指、手首

	//2017年版
	//右手	Cid=0x00+1~6 	親指、人差指、中指、薬指、子指、手首
	//左手	Cid=0x10+1~6	親指、人差指、中指、薬指、子指、手首

	//2019年版
	//右手	Cid=0x30+ 1~16 	親指、人差指、中指、薬指、子指、掌
	//左手	Cid=0x40+ 1~16	親指、人差指、中指、薬指、子指、掌

	//2020年版
	//右手	Cid=0x30+ 1~9 	親指、人差指、中指
	//左手	Cid=0x40+ 1~9	親指、人差指、中指
	...
	//3msぐらいで呼び出されている
	//vInitHardwareで初期化
	//vReceiveIoSettingRequestでデータを受ける

	//AreaID=0x03-0x04
			//DATA		mixdata[0]		0
			//MASK		mixdata[1]		0
			//PWM[0]	mixdata[2]      //5 DATA5H4     thum1(tip)
										//5 DATA5L4     thum2
			//PWM[0]	mixdata[3]    	//6 DATA6H4     thum3
										//6 DATA6L4     index1(tip)
			//PWM[1]	mixdata[4]    	//7 DATA7H4     index2
										//7 DATA7L4     index3
			//PWM[1]	mixdata[5]  	//8 DATA8H4     mid1(tip)
										//8 DATA8L4     mid2
			//PWM[2]	mixdata[6]   	//9 DATA9H4     mid3
		//以下はリザーブ
			//9 DATA9L4     ring1(tip)
			//PWM[2]	mixdata[7]    	//10 DATA10H4   ring2
										//10 DATA10L4   rin3
			//PWM[3]	mixdata[8]     	//11 DATA11H4   child1(tip)
										//11 DATA11L4   child2
			//PWM[3]	mixdata[9]     	//12 DATA12H4   child3
			//						  	//12 DATA12L4   parlm
...
//todo
	...
	MPWM=FALSE;
	for (ChNo=0;ChNo<9;++ChNo){	//親指から中まで
		if(Mpwm[ChNo]>0x00){
			if(Mpwmwait[ChNo]>0) Mpwmwait[ChNo]--;
			else{
				Mpwmwait[ChNo]=MPWM_WAIT;
				if(Mpwm[ChNo]>MPWM_REDUCE) Mpwm[ChNo]-=MPWM_REDUCE;
				else Mpwm[ChNo]=0;
				MPWM=TRUE;
//				vfPrintf(&sSerStream,"PWM=%d ",Mpwm[ChNo]);
			}
		}
		else Mpwm[ChNo]=0;
	}
////	if(MPWM) vfPrintf(&sSerStream," "LB);
//	if(MPWM) bSMBusWrite(0x70,0x82, 16, (uint8*)&Mpwm[0]); //16byte送信  PWM0 LED bright
	if(MPWM){
		if(MisLeft){
			uint8 dd[9];
			for (ChNo=0,ChNoOP=8;ChNo<9;++ChNo,--ChNoOP) dd[ChNo]=Mpwm[ChNoOP];	//降順入れ替え
			bSMBusWrite(0x70,0x82, 9, (uint8*)&Mpwm[0]); //9byte送信  PWM0 LED bright
		}
		if(MisRight){
			uint8 dd[9];
			for (ChNo=0,ChNoOP=8;ChNo<9;++ChNo,--ChNoOP) dd[ChNo]=Mpwm[ChNoOP];	//降順入れ替え
			bSMBusWrite(0x70,0x89, 9, (uint8*)&dd[0]); //9byte送信  PWM7 LED bright

		}
	}
////	if(MPWM) bSMBusWrite(0x70,0x02, 16, (uint8*)&Mpwm[0]); //16byte送信  PWM0 LED bright
}
...
static void vInitHardware(int f_warm_start) {
	...
	for (i = 0; i < 4; i++) {
		...
		//20181216 over write
		if(i>=2){
			u16PWM_Hz =  init_PWM_Hz[i]; // PWM周波数
			PUNCH=sAppData.sFlash.sData.u32PWM_Hz[0];	//Max 128
			if(PUNCH>128) PUNCH=128;
			CLIB=sAppData.sFlash.sData.u32PWM_Hz[1];	//Max 255 center 128
			if(CLIB>255) CLIB=255;
			HARDTOUCHPOS=sAppData.sFlash.sData.u32PWM_Hz[2];
			RELEASEPOS=sAppData.sFlash.sData.u32PWM_Hz[3];
			MPWM_WAIT=sAppData.sFlash.sData.u32PWM_Hz[2];
	...
	//2020年版
	//右手	Cid=30+ 1~9 	親指、人差指、中指
	//左手	Cid=40+ 1~9	親指、人差指、中指
	...
	//uint16 VIBE_Hz = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[0]);
	//uint16 VIBEAMP = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[1]);
	//uint16 VIBEAMPMAX = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[2]);

	for (i = 0; i < 16; i++){
		Mpwm[i]=0;
		Mpwmwait[i]=MPWM_WAIT;
	}
	//LDRx = 10,LED driver x individual brightness can be controlled through its PWMx register
	Mdrv[0]=0xAA;	//b10101010
	Mdrv[1]=0xAA;
	Mdrv[2]=0xAA;
	Mdrv[3]=0xAA;
...
//	u8reg=0x81;
	//MODE1
	u8reg=0x01;	//7:AutoInc_disebled,4:Nomalmode,=default
	bOk &= bSMBusWrite(0x70,0x00, 1, &u8reg);
	if(!bOk) vfPrintf(&sSerStream,"mode1 NG"LB);
	else     vfPrintf(&sSerStream,"mode1 OK"LB);

	//LEDOUT0
////	bOk &= bSMBusWrite(0x70,0x14, 4, (uint8*)&Mdrv[0]);
	bOk &= bSMBusWrite(0x70,0x94, 4, (uint8*)&Mdrv[0]);
	if(!bOk) vfPrintf(&sSerStream,"LED DRIVE NG"LB);
	else     vfPrintf(&sSerStream," OK"LB);

	//PWM0
//	bOk &= bSMBusWrite(0x70,0x82, 2, (uint8*)&Mpwm[0]); //PWM0 LED bright
//	if(!bOk) vfPrintf(&sSerStream,"LED DRIVE NG"LB);
//	else     vfPrintf(&sSerStream," OK"LB);
...
static void vSerUpdateScreen() {
...

	{
		...
		V_PRINT(" z: set VFreq/Punch,AmpLMT/Clib,HardTouchPos/MPWM_WAIT,ReleasePos (");
		for (i = 0; i < 4; i++) {
			if (i) V_PUTCHAR(',');
...
//				V_PRINT("* Ch=%d, Role=%d, Layer=%d"LB,
				V_PRINT("* Ch=%d, Role=%d, HardTouchPos=%d"LB,
...
static void vReceiveIoData(tsRxDataApp *pRx) {
	...
	for (i = 0; i < 4; i++) {
		if (sAppData.sIOData_now.au16OutputPWMDuty[i] != 0xFFFF) {
			sTimerPWM[i].u16duty =_PWM(sAppData.sIOData_now.au16OutputPWMDuty[i]);
...
static void vReceiveIoSettingRequest(tsRxDataApp *pRx) {
	int i, j;
	uint8 *p = pRx->auData;
	//	const float kk[3]={179.712766168243,9.08382722921235,0.0157450765826735}; //Wn,C,ita
	const float kk[3]={156.16,9.08382722921235,0.00822589605414518};
	...
	if ((u8Format& 0x0f) == 1) {
		if( (u8Format & 0xF0) ==0){		//nomal TWELITE
			/* BUTTON */
			uint8 u8ButtonState = G_OCTET();
			uint8 u8ButtonChanged = G_OCTET();
			// ポートの値を設定する（変更フラグのあるものだけ）
			...
			if (sAppData.sIOData_now.au16OutputPWMDuty[0] != 0xFFFF && sAppData.sIOData_now.au16OutputPWMDuty[1] != 0xFFFF) {
	//			const float kk[3]={156.16,9.08382722921235,0.00822589605414518};

				uint16 VIBE_Duty=512;
				uint16 VIBE_Hz = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[0]);
				uint16 VIBEAMP = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[1]);
				uint16 VIBEAMPMAX = _PWM(sAppData.sIOData_now.au16OutputPWMDuty[2]);
				uint16 VIBEAMPDuty=1024;//_PWM(0);

				if( VIBEAMP!=0){
					//float wn=(float)VIBE_Hz/179.71;
					float wn=(float)VIBE_Hz/kk[0];
		//			float VIBEAMPDuty_=(float)VIBEAMP*2.2592*sqrt((1-wn*wn)*(1-wn*wn)+0.015772089*wn*wn);
					float VIBEAMPDuty_=(float)VIBEAMP*kk[1]*sqrt((1-wn*wn)*(1-wn*wn)+kk[2]*wn*wn);


					if (VIBEAMPDuty_<0.0){		//low power stop
		//				VIBE_Hz=100;
						VIBE_Duty=_PWM(0);
						VIBEAMPDuty=1022;//_PWM(0);
					}
					else if  (VIBEAMPDuty_>(float)VIBEAMPMAX)
							VIBEAMPDuty=1024-VIBEAMPMAX;	//High power limit
					else	VIBEAMPDuty=1024-(uint16)VIBEAMPDuty_;

					if (VIBE_Hz<10){
		//				VIBE_Hz=0xFFFF;
						VIBE_Duty=_PWM(0);
						VIBEAMPDuty=1024;//_PWM(0);
					}
				}
				else{
					VIBEAMPDuty=1024-VIBEAMPMAX;
	//				VIBEAMPDuty=0;//1024-VIBEAMPMAX;
				}
				sTimerPWM[0].u16Hz = VIBE_Hz;
				sTimerPWM[0].u16duty=VIBE_Duty;
				sTimerPWM[1].u16duty =VIBEAMPDuty;

				vfPrintf(&sSerStream,"Gain=%d, Hz=%d, A=%d "LB,1024-VIBEAMPDuty,VIBE_Hz,VIBEAMP);
	//			vfPrintf(&sSerStream,"Gain=%d"LB,(int)(wn*1000));

			}
			else{
				sTimerPWM[0].u16Hz =100;
				sTimerPWM[0].u16duty=_PWM(0);
				sTimerPWM[1].u16duty =1024;//_PWM(0);
				vfPrintf(&sSerStream,"Gain=0, Hz=XX, A=0 "LB);
			}

			if (sTimerPWM[0].bStarted && sTimerPWM[1].bStarted){
				vTimerChangeHz(&sTimerPWM[0]);
				vTimerStart(&sTimerPWM[0]); // DUTY比だけ変更する
				vTimerStart(&sTimerPWM[1]); // DUTY比だけ変更する
			}
			else{
				vfPrintf(&sSerStream,"Now stoped "LB);
			}

			if (sAppData.sIOData_now.au16OutputPWMDuty[3] != 0xFFFF){
				sTimerPWM[3].u16duty =_PWM(sAppData.sIOData_now.au16OutputPWMDuty[3]);
//				if(sTimerPWM[3].u16duty>RELEASEPOS) sTimerPWM[3].u16duty=RELEASEPOS;
//				if(sTimerPWM[3].u16duty<HARDTOUCHPOS) sTimerPWM[3].u16duty=HARDTOUCHPOS;
			}
			else
				sTimerPWM[3].u16Hz=0xFFFF;
			if (sTimerPWM[3].bStarted)	vTimerStart(&sTimerPWM[3]); // DUTY比だけ変更する
	//		vfPrintf(&sSerStream,"*");
		}
//s2018
		//2020年版
...
/*
		#define SOFT_TOUCH_POS  92
		#define RELEASE_POS  112
		#define TOUCHVIBE 66	//=200ms
		if(u8ButtonState>0){
			int MdrvID,sftr,MtID,MtBID;
			MdrvID=(u8ButtonState-1)/4;
			sftr=(u8ButtonState-1)%4*2;
			MtID=(u8ButtonState-1)/3;
			MtBID=(u8ButtonState-1)%3;
			Mdrv[MdrvID]|=(0x02<<sftr);
			//周波数コマンドなら
			if(sTimerPWM[0].u16duty<0xFFFF && sTimerPWM[0].u16duty>0){
				if(Mtimer[MtID][MtBID]<sTimerPWM[0].u16duty) Mtimer[MtID][MtBID]=sTimerPWM[0].u16duty;
			}
			//接触コマンドなら
			if(sTimerPWM[3].u16duty<RELEASE_POS){
				if(Mtimer[MtID][MtBID]<TOUCHVIBE) Mtimer[MtID][MtBID]=TOUCHVIBE;
			}
		}*/


        //AreaID
        //0x01  righthand fingertips
        //0x02  lefthand fingertips
        //0x03	righthand fingerothers only vibration
        //0x04	lefthand fingerothers only vibration

		//Freq
		//入力 0~255を2倍して0~510とし	0はstopでoxFFFF	残りは+9して10~519Hzとする
		//Amp
		//入力 0~15を64倍して0~960umとし、	0はstopでoxFFFF	残りを64~960umとする
		//Pos
		//入力 0~15を5倍して0~75ととし	0はstopでoxFFFF	残り5~75を117から引く
        //const ushort RELEASE_POS    =112;     117-5=112	->1
        //const ushort SOFT_TOUCH_POS = 92;     117-25=92	->5
        //             MID            = 56;     117-60=57	->12
        //const ushort HARD_TOUCH_POS = 41;     117-75=42	->15

		//ID		broardbandID	0x78
		//CMD		0x80
		//Cformat   (AreaID<<4)+0x01
//AreaID=0x01-0x02
		//DATA		mixdata[0]		//3 DATA3_8     thumFreq   0:stop 1~255->10~519Hz
		//MASK		mixdata[1]		//4 DATA4_8     indexFeq
		//PWM[0]	mixdata[2]      //5 DATA5_8     midFreq
		//PWM[0]	mixdata[3]    	//6 DATA6_8     ringFreq
		//PWM[1]	mixdata[4]    	//7 DATA7_8     childFreq
		//PWM[1]	mixdata[5]  	//8 DATA8H4     thumAmp    0~15<0~960um
									//8 DATA8L4     thumPos
		//PWM[2]	mixdata[6]   	//9 DATA9H4     indexAmp
									//9 DATA9L4     indexPos
		//PWM[2]	mixdata[7]    	//10 DATA10H4   midAmp
									//10 DATA10L4   midPos
		//PWM[3]	mixdata[8]     	//11 DATA11H4   ringAmp
									//11 DATA11L4   ringPos
		//PWM[3]	mixdata[9]     	//12 DATA12H4   childAmp
		//						  	//12 DATA12L4   childPos
//AreaID=0x03-0x04
		//DATA		mixdata[0]		0
		//MASK		mixdata[1]		0
		//PWM[0]	mixdata[2]      //5 DATA5H4     thum1(tip)
...
//e2018
			...
			//AreaID
			//0x01  righthand fingertips	OK
			//0x02  lefthand fingertips		OK
			//0x03	righthand fingerothers only vibration
			//0x04	lefthand fingerothers only vibration

			uint8 AreaID=0x0F & (u8Format>>4);

			if( AreaID==0x02||AreaID==0x01){

				if ( AreaID != sAppData.u8AppLogicalId/10+1){
					return;
				}
				uint8 SubID=sAppData.u8AppLogicalId %10;	// 1~5 (thum~child), and 6 (wrist)

				uint8 mixdata[10];
				for (i = 0; i < 10; i++) mixdata[i] = G_OCTET();

//				#define  HARD_TOUCH_POS  41
//				#define	RELEASE_POS 112
//				#define	BASE_POS RELEASE_POS+5

				uint16 VIBE_Duty=512;
//				uint16 VIBE_Hz =sTimerPWM[0].u16Hz;
				uint16 VIBE_Hz =0;
				uint16 VIBEAMP =0;
//				uint16 VIBEAMPMAX =sTimerPWM[2].u16Hz;
				uint16 VIBEAMPMAX =1023;
				uint16 VIBEAMPDuty=1024;//_PWM(0);
				uint16 VIBE_pos=RELEASEPOS;

				//SubID 1~5 (thum~child)
				if(SubID>0 && SubID<6){
					VIBE_Hz=((uint16)mixdata[SubID-1]<<1);
					if(VIBE_Hz>0) VIBE_Hz+=9;
					VIBEAMP=((uint16)(mixdata[SubID+4]>>4) & 0x0F)<<6;		//mixdata[5~9]
					VIBE_pos=RELEASEPOS+5-(((mixdata[SubID+4]>>0) & 0x0F)*5);
				}
				if(VIBE_Hz>1024 || VIBE_Hz==0) VIBE_Hz= 0xFFFF;
				if(VIBEAMP>1024 || VIBEAMP==0) VIBEAMP= 0xFFFF;
				if(VIBE_pos>RELEASEPOS) VIBE_pos= 0xFFFF;
				if(VIBE_pos<HARDTOUCHPOS) VIBE_pos=HARDTOUCHPOS;
		...
		//		if (sAppData.sIOData_now.au16OutputPWMDuty[0] != 0xFFFF && sAppData.sIOData_now.au16OutputPWMDuty[1] != 0xFFFF) {
				if ((VIBE_Hz != 0xFFFF) && (VIBEAMP != 0xFFFF)) {
		...
		//				VIBEAMPDuty=0;//1024-VIBEAMPMAX;
					...
					vfPrintf(&sSerStream,"Gain=%d, Hz=%d, A=%d P=%d AID=%d SID=%d"LB,1024-VIBEAMPDuty,VIBE_Hz,VIBEAMP,VIBE_pos,AreaID,SubID);
				...
				if (VIBE_pos!= 0xFFFF)
					sTimerPWM[3].u16duty =VIBE_pos;
				else
					sTimerPWM[3].u16Hz=0xFFFF;
				if (sTimerPWM[3].bStarted)	vTimerStart(&sTimerPWM[3]); // DUTY比だけ変更する
		//		vfPrintf(&sSerStream,"*");
			}
			else if( AreaID==0x03||AreaID==0x04){	//AreaID==0x03||AreaID==0x04
				if ( AreaID != ((uint16)(sAppData.u8AppLogicalId>>4)&0x0f)){
//					vfPrintf(&sSerStream,"wrong ID "LB);
					return;
				}

//				#define PUNCH 16
				uint8 mixdata[10],Mpwm_;
				for (i = 0; i < 10; i++) mixdata[i] = G_OCTET();
				for (i = 0; i < 8; i++){
					Mpwm_=((uint8)(mixdata[i+2]&0xf0)>>1);
					Mpwm_=(uint8)(((uint16)Mpwm_*CLIB)>>7); //Mpwm_=Mpwm_*CLIB/128
					if(Mpwm_+(uint8)PUNCH>Mpwm[i*2+0]&&Mpwm_>0){
						Mpwm[i*2+0]=Mpwm_+(uint8)PUNCH;
						vfPrintf(&sSerStream,"No%d %d %d %d "LB,i*2+0,Mpwm_,mixdata[i+2],PUNCH);// ((uint16)(sAppData.u8AppLogicalId>>4)&0x0f));
					}
					Mpwm_=((uint8)(mixdata[i+2]&0x0f)<<3);
					Mpwm_=(uint8)(((uint16)Mpwm_*CLIB)>>7);	//Mpwm_=Mpwm_*CLIB/128
					if(Mpwm_+(uint8)PUNCH>Mpwm[i*2+1]&&Mpwm_>0){
						Mpwm[i*2+1]=Mpwm_+(uint8)PUNCH;
						vfPrintf(&sSerStream,"No%d %d %d  %d "LB,i*2+1,Mpwm_,mixdata[i+2],PUNCH);// ((uint16)(sAppData.u8AppLogicalId>>4)&0x0f));
					}
				}
			}
			else return;
...
//	p->u8role = 41;		//hard touch pos
//	p->u8layer = 112;	//release touch pos

	p->u16SleepDur_ms = 1000;
	p->u16SleepDur_s = (MODE7_SLEEP_DUR_ms / 1000);
	p->u8Fps = 32;

	p->u32PWM_Hz[0] = 16;		//Freq 100 	Punch 8
	p->u32PWM_Hz[1] = 128;		//AMP 128	Clibration 128
	p->u32PWM_Hz[2] = 1;		//hardtouchpos 42	MPWM_WAIT 1
	p->u32PWM_Hz[3] = 112;		//releasepos
								//duty 4%～11％が動作範囲 40(CW)～112(CCW) 約170deg
								//duty 7%～11％が約90deg
